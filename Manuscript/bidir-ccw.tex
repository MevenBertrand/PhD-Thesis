...
the \reintro{subject},
whose well-formation is under scrutiny, \reintro{inputs},
...
\begin{figure*}
  % \hfsetfillcolor{black!8}
  \begin{mathpar}
  \inferrule[(1)]
    {\phantom{\Gamma \vdash t \pity{\P} \P x : A. B}}
    {\Gamma \vdash \tikzmarkin{s1} t\ u \tikzmarkend{s1} \ity \phantom{\subs{B}{x}{u}}} \and
  \inferrule[(2)]
    {\Gamma \vdash \tikzmarkin{p1} t \tikzmarkend{p1} \pity{\P} \phantom{\P x : A. B}}
    {\Gamma \vdash t\ u \ity \phantom{\subs{B}{x}{u}}} \and
  \inferrule[(3)]{\Gamma \vdash t \pity{\P} \P x : A. B}
  {\Gamma \vdash t\ u \ity \subs{B}{x}{u}} \and
  \inferrule[(4)]
  {\Gamma \vdash t \pity{\P} \P x : A. B \\
    \Gamma \vdash \tikzmarkin{p2} u \tikzmarkend{p2} \cty A}
  {\Gamma \vdash t\ u \ity \subs{B}{x}{u}} \and
  \inferrule[(5)]
  {\Gamma \vdash t \pity{\P} \P x : A. B \\ \Gamma \vdash u \cty A}
  {\Gamma \vdash \tikzmarkin{s2} t\ u \tikzmarkend{s2} \ity} \and
  \inferrule[(6)]
  {\Gamma \vdash t \pity{\P} \P x : A. B \\ \Gamma \vdash u \cty A}
  {\Gamma \vdash t\ u \ity \subs{B}{x}{u}}
  \end{mathpar}
...
Interestingly, a somewhat similar discipline has appeared independently in
...
consider well-formation of the objects, only their knownledge. For instance, that criterion
would allow for $\l x : A.\ t$ to directly extend a context with $A$ to infer a type for
$t$, because it is known, but McBride’s discipline forbids it, because $A$’s well-formation
is not established.
...
The first, which is the usual one in \kl{Pure Type Systems} – \kl{PTS}, see
\sidetextcite{Barendregt1991} –,
...
as an input to the conclusion, it is not useful to re-check it. This is also in line with implementations, where the context is not re-checked at leaves of a derivation tree, with performance issues in mind. The well-formation invariants then ensure that any derivation starting with the (well-formed) empty context will only ever encounter well-formed contexts.
...
which can nevertheless be retrieved by using well-formation invariants.
Thus, we get the following correctness theorem – note how McBride’s discipline manifests as well-formation hypothesis on inputs.
