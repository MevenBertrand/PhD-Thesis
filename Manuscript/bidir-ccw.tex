...
As an illustration, an example of a rule that does respect this discipline – that
...
which we introduce soon, and focus on well-formation: objects known to be well-formed
are on a blue background, those which are not are on a red one.
First, $\Gamma$ is well-formed, as an input to the conclusion (1), and
...
not enough: since $\subs{B}{x}{u}$ is an output to the conclusion,
...
which is more structured, and thus more amenable to proofs.
