...
\kl{cumulativity} check, and \cref{sec:kernel-typing} describes the type-checker properly said.
\Cref{sec:kernel-beyond-typing} describes two extra functions belonging to the safe
...
\kl{Coq}’s inductive types: it is both mutual, and nested – due to the presence of lists,
records and contexts which recursively containing terms.
While \kl{Coq} is able to detect the definition to be well-founded,
the default generation is unable to derive a sensible induction principle, and neither are
the \coqe{Scheme} specialized commands. Thus, we have to prove a good induction principle by
hand. To do so, we introduce a notion of “generic” typing object
\pcuicline[Bidirectional][BD]{Typing}{typing\_sum}{256}, together with a notion of size for such
