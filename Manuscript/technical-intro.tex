...
\begin{property}[Stability under renaming]
  ...
  \end{itemize} 
  we have $\Delta \vdash \multisubs{t}{x_1 \into y_1 \dots x_n \into y_n} \ty \multisubs{T}{x_1 \into y_1 \dots x_n \into y_n}$.
\end{property}
...
\begin{property}[Weakening]
  ...
  A type theory is said to satisfy \intro{weakening} if whenever $\Gamma \vdash t \ty T$
  and $\Gamma \vdash A \ty \uni$ we have $\Gamma, x : A \vdash t \ty T$.
\end{property}
...
\begin{property}[Stability under substitution]
  ...
  \end{itemize} 
  we have $\Delta \vdash \multisubs{t}{\sigma} \ty \multisubs{T}{\sigma}$.
\end{property}
...
\begin{property}[Strengthening]
  ...
  $\Gamma, x : A \vdash t \ty T$ and $x$ does not appear in $t$,
...
\begin{property}[Validity]
  \label{prop:validity}
  A type theory satisfies \intro{validity} if whenever $\Gamma \vdash t \ty T$, we have
  $\vdash \Gamma$ and $\Gamma \vdash T \ty \uni$.
\end{property}
...
\begin{property}[Subject reduction]
  ...
  $\Gamma \vdash t \ty T$ and $t \red t'$, then also $\Gamma \vdash t' \ty T$.
\end{property}
...
\begin{definition}[Accessibility]
  Let $R$ be a relation on $A$. An inhabitant $a$ of $A$ is \intro{accessible} if all $a'$
...
\begin{property}[Normalization]
  ...
  A type system has the \intro{normalization} property if every
  well-typed term is \kl{accessible} for one-step co-reduction $\ocored$, the
  inverse relation of \kl{one-step reduction} $\ored$.
\end{property}
