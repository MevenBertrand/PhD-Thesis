...
\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
  \inferdef{Var}{\vdash \Gamma \\ (x : A) \in \Gamma}{\Gamma \vdash x : A}
  \label{rule:cic-var}
  \end{mathpar}
...
\begin{marginfigure}
  \begin{mathpar}
    \inferrule{\Gamma \vdash A : \uni \\ \Gamma, x : A \vdash t : T}
    {\Gamma \vdash \l x : A .\ t : A \to T}
    \and
...
\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
    \inferdef{Abs}{\Gamma \vdash A : \uni \\ \Gamma, x : A \vdash t : T}
    {\Gamma \vdash \l x : A .\ t : \P x : A.\ T}
    \label{rule:cic-abs}
    \and
...
\begin{property}[Stability under renaming]
  \label{prop:stab-renaming}
  A type theory is said to be \intro{stable under renaming}
...
\begin{property}[Weakening]
  ...
  A type theory is said to satisfy \intro{weakening} if whenever $\Gamma \vdash t : T$
...
\begin{property}[Stability under substitution]
  \label{prop:stab-subst}
  A type theory is said to be \intro{stable under substitution}
...
\begin{property}[Conditional stability under renaming]
  \label{prop:strong-stab-renaming}
  A type theory is said to satisfy \intro{conditional stability under renaming}
  if whenever the following conditions are met
  \begin{itemize}
    \item $x_1 : A_1 \dots x_n : A_n \vdash t : T$
    \item $\vdash \Delta$
...
\begin{property}[Strengthening]
  ...
  A type theory is said to satisfy \intro{strengthening} if whenever
...
\begin{property}[Validity]
  \label{prop:validity}
  A type theory satisfies \intro{validity} if whenever $\Gamma \vdash t : T$, we have
...
\ruleref{rule:cic-var} or $\Gamma \vdash T' : \uni$ in \ruleref{rule:cic-conv-unty}.
...
\begin{property}[Uniqueness of types]
  \label{prop:uniqueness}
  A type theory satisfies \intro{uniqueness of types up to} a relation $\sqsubseteq$
...
\begin{property}[Subject reduction]
  \label{prop:sr}
  The \intro{subject reduction} property – also called \kl{preservation} –
...
deduce $\Gamma \vdash u : A$, then using \kl{stability under
substitution} we would get $\Gamma \vdash \subs{t}{x}{u} : \subs{B}{x}{u}$,
which would finally lead to $\Gamma \vdash \subs{t}{x}{u} : T$ using \kl{stability under
substitution} and transitivity of conversion. Thus, the key property is the following:

\begin{property}[Injectivity of product types]
  \label{prop:prod-inj}
  A conversion relation $\conv$ is said to have \intro{injectivity of product types}
...
For \kl{declarative conversion}, transitivity is trivial, but \kl{injectivity of product types}
...
Conversely, for \kl{algorithmic conversion}, \kl{injectivity of product types} is rather
...
\begin{property}[Confluence]
  ...
  The \intro{confluence} property of reduction says that whenever
...
\begin{property}[Progress]
  A type system is said to satisfy \intro{progress} – given a notion of \kl{normal forms} –
...
forever, but that instead \kl{reduction} eventually terminates.
We carefully phrase this is a constructive manner, as follows:

\begin{property}[Normalization]
  \label{prop:normalization}
  A term is called \kl{normalizing} if it is accessible%
  \sidenote{An object $a : A$ is \intro{accessible} for a relation $R$ if all $a'$
  such that $a \mathrel{R} a'$ are themselves accessible.} 
  for \kl{reduction}.
  A type system has the \intro{normalization} property if every well-typed term is
  normalizing.
\end{property}
...
\begin{property}[Logical consistency]
  \label{prop:log-cons}
  A type system is \intro{logically consistent} if there is a type which is uninhabited
...
\begin{property}[Canonicity]
  ...
  A type system has the \intro{canonicity} property
...
\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
    \inferdef{BoolTy}{\vdash \Gamma}{\Gamma \vdash \Bool : \uni[0]}
...
\begin{marginfigure}
  ...
  \begin{mathpar}
    ...
    \label{rule:false}
    \inferdef{True}{\vdash \Gamma}{\Gamma \vdash \true : \Bool}
    \label{rule:true}
  \end{mathpar}
...
\begin{figure}
  \begin{mathpar}
    ...
    {\Gamma \vdash \pair[A][x.B]{t}{u} : \Sb x : A.\ B}
    \label{rule:pair-type} \and
    \inferdef{PairInd}
      {\Gamma \vdash s : \Sb x : A.\ B \\
      \Gamma, z : \Sb x : A.\ B \vdash P : \uni \\
      \Gamma, y_1 : A, y_2 : \subs{B}{x}{y_1} \vdash b : \subs{P}{z}{\pair[A][x.B]{y_1}{y_2}}}
      {\Gamma \vdash \ind{\Sb}{s}{z.P}{y_1.y_2.b} : \subs{P}{z}{s}}
...
\begin{figure*}
  \begin{mathpar}
    ...
    \inferdef{VectInd}
      ...
      \Gamma, y_1 : \Nat, y_2 : A, y_3 : \Vect(A,y_1), p_{y_3} : \multisubs{P}{y \into y_1, z \into y_3} \vdash
        b_{\vconsop} : \multisubs{P}{y \into \S{y_1}, z \into \vcons[A][y_1]{y_2}{y_3}}}
      {\Gamma \vdash \ind{\Vect}{s}{y.z.P}{b_{\vnil},y_1.y_2.y_3.p_{y_3}.b_{\vconsop}}
        : \multisubs{P}{y \into n, z \into s}}
...
\begin{marginfigure}
  \begin{mathpar}
    ...
    \inferdef{ΠTyProp}{\Gamma \vdash A : \uni \\ \Gamma, x : A \vdash P : \Prop}
    {\Gamma \vdash \P x : A.\ P : \Prop}
    \label{rule:prop-prod} \and
    \inferdef{ΠPropProp}{\Gamma \vdash A : \Prop \\ \Gamma, x : A \vdash P : \Prop}
    {\Gamma \vdash \P x : A.\ P : \Prop}
...
\begin{figure}
  ...
  \begin{mathpar}
    ...
    \Gamma, x \coloneqq t : A \vdash u : B}
