...
$\Gamma \vdash t \colon T$, which reads "in the context $\Gamma$, the term $t$ has type $T$".
...
Hence, $\Gamma$ is a list of declarations, of the form $x \colon A$.
We write $\emptycon$ for the
empty context, $\Gamma, x \colon A$ for the extension of context $\Gamma$ with the new variable $x \colon A$, and $(x \colon A) \in \Gamma$ to denote that the declaration $x \colon A$ appears at some
point in the context $\Gamma$.

\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
  \inferdef{Var}{(x \colon A) \in \Gamma \\ \vdash \Gamma}{\Gamma \vdash x \colon A}
  \label{rule:cic-var}
  \end{mathpar}
...
\ie from $(x \colon A) \in \Gamma$
and $\vdash \Gamma$ – a judgement that will soon be defined asserting that the context
$\Gamma$ is well-formed – we can deduce $\Gamma \vdash x \colon A$.
...
rules. Equivalently, a judgement such as $\Gamma \vdash t \colon T$
...
\begin{marginfigure}
  \begin{mathpar}
    \inferrule{\Gamma, x \colon A \vdash t \colon T}{\Gamma \vdash \l x \colon A .\ t \colon A \to T}
    \and
    \inferrule{\Gamma \vdash f \colon A \to T \\ \Gamma \vdash u \colon A }{ \Gamma \vdash t\ u \colon T}
  \end{mathpar}
  \caption{Typing for non-dependent functions}
  \label{fig:cic-nondep-fun}
\end{marginfigure}

Functions, also called λ-abstractions, are written $\l x \colon A .\ t$. This corresponds
...
\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
    \inferdef{Abs}{\Gamma, x \colon A \vdash t \colon T}{\Gamma \vdash \l x \colon A .\ t \colon \P x \colon A.\ T}
    \label{rule:cic-abs}
    \and
    \inferdef{App}{\Gamma \vdash f \colon \P x \colon A.\ T \\ \Gamma \vdash u \colon A }{ \Gamma \vdash f\ u \colon \subs{T}{x}{u}}
    \label{rule:cic-app}
  \end{mathpar}
...
$\operatorname{\forall} x \colon A.\ T(x)$.
Indeed, if one can show that $T(x)$ holds for an unspecified $x$,
then it must hold for all $x \colon A$ – this is \ruleref{rule:cic-abs}.
Conversely, if $T$ holds for all $x \colon A$, then one can deduce $T(u)$ for any specific
$u \colon A$ – this is \ruleref{rule:cic-app}.
...
$A \to T$ is shorthand for $\P x \colon A.\ T$ when $T$ does not mention $x$.
...
\begin{marginfigure}
  ...
  \begin{mathpar}
    \inferdef{Univ}
    {\vdash \Gamma}
    {\Gamma \vdash \uni[i] \colon \uni[\unext{i}]}
    \label{rule:cic-univ}
  \end{mathpar}
...
Martin-Löf~\sidecite{MartinLoef1972}, which has a single universe $\uni$ and a rule $\uni \colon \uni$.
...
\begin{marginfigure}
  ...
  \begin{mathpar}
    \inferdef{Prod}
    {\Gamma \vdash A \colon \uni[i] \\ \Gamma, x \colon A \vdash B \colon \uni[j]}
    {\Gamma \vdash \P x \colon A.\ B \colon \uni[\umax{i}{j}]}
    \label{rule:cic-prod}
  \end{mathpar}
...
\begin{marginfigure}
  ...
  \begin{mathpar}
    ...
    \inferdef{ConsCon}
    {\vdash \Gamma \\ \Gamma \vdash A \colon \uni}{\vdash \Gamma, x \colon A}
...
\begin{marginfigure}
  ...
  \begin{mathpar}
  \inferdef{Conv}
    {\Gamma \vdash t \colon T \\ \Gamma \vdash T \conv T' \colon \uni}
    {\Gamma \vdash t \colon T'}
  \label{rule:cic-conv}
  \end{mathpar}
...
\begin{figure*}[ht]
  ...
  \begin{mathpar}
    ...
    \and
    \inferdef{App}
      {\Gamma \vdash f \colon \P x \colon A.\ T \\ \Gamma \vdash u \colon A }
      {\Gamma \vdash f\ u \colon \subs{T}{x}{u}}
    \and
  \inferdef{Conv}
    {\Gamma \vdash t \colon T \\ \Gamma \vdash T \conv T' \colon \uni}
    {\Gamma \vdash t \colon T'}
...
difference is in how the relation $\Gamma \vdash T \conv T' \colon \uni$ is defined.
...
The "content" of the two rules is the same – they equate $(\l x \colon A.\ t)\ u$
...
\begin{figure}[ht]
  \begin{mathpar}
    \inferrule
      {\Gamma, x \colon A \vdash t \colon B \\ \Gamma \vdash u \colon A}
      {\Gamma \vdash (\l x \colon A.\ t)\ u \tdconv \subs{t}{x}{u} \colon \subs{B}{x}{u}}
    \and
    \inferrule{ }{(\l x \colon A.\ t)\ u \udconv \subs{t}{x}{u}}
  \end{mathpar}
...
\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
    \inferdef{UConv}{\Gamma \vdash T' \colon \uni \\ T \udconv T'}{\Gamma \vdash T \conv T' \colon \uni}
...
in order to ensure that, whenever $\Gamma \vdash t \colon T$ is derivable,
$\Gamma \vdash T \colon \uni$ is as well.
...
\begin{marginfigure}
  \ContinuedFloat
  \begin{mathpar}
    \inferdef{βConv}{ }{(\l x \colon A.\ t)\ u \udconv \subs{t}{x}{u}}
...
\begin{figure}[hb]
  ...
  \begin{mathpar}
    ...
    % \inferdef{ProdConv}
      ...
      {\P x \colon A.\ B \udconv \P x \colon A'.\ B'}
    % \label{rule:cic-uconv-prod}
    \and
    \inferrule
    % \inferdef {AbsConv}
      {A \udconv A' \\ t \udconv t'}
      {\l x \colon A .\ t \udconv \l x \colon A'.\ t'}
...
\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
    \inferdef{βRed}{ }{(\l x \colon A.\ t)\ u \tred \subs{t}{x}{u}}
    \label{rule:beta-red}
  \end{mathpar}
...
\begin{figure}[ht]
  ...
  \begin{mathpar}
    ...
    % \inferdef{ProdRedDom}
      {A \ored A'}
      {\P x \colon A.\ B \ored \P x \colon A'.\ B}
    ...
    % \inferdef{AbsRedDom}
      {A \ored A'}
      {\l x \colon A .\ t \ored \l x \colon A'.\ t}
...
\begin{figure}[ht]
  ...
  \begin{mathpar}
    \inferdef{AlgConv}
    {\Gamma \vdash T \colon \uni \\ T \red U \\ T' \red U' \\ U \alpheq U' }
    {\Gamma \vdash T \conv T' \colon \uni}
    \label{rule:alg-conv}
  \end{mathpar}
...
\begin{itemize}
  ...
  \item \intro{subject reduction} (confluence, injectivity of type constructors)
  \item \intro{Normalization}
...
\begin{marginfigure}
  \ContinuedFloat*
  \begin{mathpar}
    \inferdef{BoolTy}{\vdash \Gamma}{\Gamma \vdash \Bool \colon \uni[0]}
...
\begin{marginfigure}
  \ContinuedFloat
  \begin{mathpar}
    \inferdef{False}{\vdash \Gamma}{\Gamma \vdash \false \colon \Bool} \and
    \label{rule:false}
    \inferdef{True}{\vdash \Gamma}{\Gamma \vdash \true \colon \Bool}
    \label{rule:true}
  \end{mathpar}
...
\sidenote{We call $s$ the \intro{scrutinee}, $P$ the \intro{predicate}
and $b_{\false}$, $b_{\true}$ the \intro{branches}.}
...
We also use the name \intro{recursor} interchangeably with induction principle,
...
\begin{figure}
  ...
  \begin{mathpar}
    \inferdef{BoolInd}
      {\Gamma \vdash s \colon \Bool \\
      \Gamma, z \colon \Bool \vdash P \colon \uni \\
      \Gamma \vdash b_{\false} \colon \subs{P}{z}{\false} \\
      \Gamma \vdash b_{\true} \colon \subs{P}{z}{\true}}
      {\Gamma \vdash \ind{\Bool}{s}{z.P}{b_{\false},b_{\true}} \colon \subs{P}{z}{s}}
      \label{rule:bool-ind}
  \end{mathpar}
...
\begin{figure}[h]
  \begin{mathpar}
    \inferdef{Nat}{\vdash \Gamma}{\Gamma \vdash \Nat \colon \uni[0]}
    \label{rule:nat-type} \and
    \inferdef{Zero}{\vdash \Gamma}{\Gamma \vdash \z \colon \Nat}
    \label{rule:zero-type} \and
    \inferdef{Succ}{\Gamma \vdash n \colon \Nat}{\Gamma \vdash \S{n} \colon \Nat}
    \label{rule:succ-type} \and
    \inferdef{NatInd}
      {\Gamma \vdash s \colon \Nat \\
      \Gamma, z \colon \Nat \vdash P \colon \uni \\
      \Gamma \vdash b_{\z} \colon \subs{P}{z}{\z} \\
      \Gamma, y \colon \Nat, p_{y} \colon \subs{P}{z}{y} \vdash b_{\Sop} \colon \subs{P}{z}{\S{y}}}
      {\Gamma \vdash \ind{\Nat}{s}{z.P}{b_{\z},y.p_{y}.b_{\Sop}} \colon \subs{P}{z}{s}}
...
as a term. \kl{Coq} is slightly more permissive, and allows $\Sop \colon \Nat \to \Nat$.
We forbid this – but one can always consider $\l x \colon \Nat.\ \S{x}$ instead if needed.
...
dependent pair type $\Sb x \colon A.\ B$. Logically, its dependency on $A$ means
...
\begin{figure}
  \begin{mathpar}
    \inferdef{PairTy}
      {\vdash A \colon \uni[i] \\ \Gamma, x \colon A \vdash B \colon \uni[j]}
      {\Gamma \vdash \Sb x \colon A.\ B \colon \uni[\umax{i}{j}]}
    \label{rule:sig-type} \and
    \inferdef{Pair}
    {\Gamma \vdash A \colon \uni \\ \Gamma, x \colon A \vdash B \colon \uni \\
    \Gamma \vdash t \colon A \\ \Gamma \vdash u \colon \subs{B}{x}{t}}
    {\Gamma \vdash \pair[A][B]{t}{u} \colon \Sb x \colon A.\ B}
    \label{rule:pair-type} \and
    \inferdef{PairInd}
      {\Gamma \vdash s \colon \Sb x \colon A.\ B \\
      \Gamma, z \colon \Sb x \colon A.\ B \vdash P \colon \uni \\
      \Gamma, y_1 \colon A, y_2 \colon \subs{B}{x}{y_1} \vdash b \colon \subs{P}{z}{\pair[A][B]{y_1}{y_2}}}
      {\Gamma \vdash \ind{\Sb}{s}{z.P}{y_1.y_2.b} \colon \subs{P}{z}{s}}
      \label{rule:sig-ind} \and
    \inferdef{ιPair}
    { }
    {\ind{\Sb}{\pair[A][B]{t}{u}}{z.P}{y_1.y_2.b} \tred \multisubs{b}{y_1 \into t, y_2 \into u}} 
    \label{rule:iota-sig}
  \end{mathpar}
...
\begin{mathpar}
  \inferdef{ListTy}
    {\vdash A \colon \uni[i]}
    {\Gamma \vdash \List(A) \colon \uni[i]}
  \label{rule:list-type} \and
  \inferdef{Nil}
    {\Gamma \vdash A \colon \uni}
    {\Gamma \vdash \lnil[A] \colon \List(A)}
  \label{rule:nil-type} \and
  \inferdef{Cons}
    {\Gamma \vdash A \colon \uni \\ 
    \Gamma \vdash a \colon A \\ \Gamma \vdash l \colon \List(A)}
    {\Gamma \vdash \lcons[A]{a}{l} \colon \List(A)}
  \label{rule:cons-type} \and
  \inferdef{ListInd}
    {\Gamma \vdash s \colon \List(A) \\
    \Gamma, z \colon \List(A) \vdash P \colon \uni \\
    \Gamma \vdash b_{\lnil} \colon \subs{P}{z}{\lnil} \\
    \Gamma, y_1 \colon A, y_2 \colon \List(A), p_{y_2} \colon \subs{P}{z}{y_2}
      \vdash b_{\lconsop} \colon \subs{P}{z}{\lcons[A]{y_1}{y_2}}}
...
\begin{figure}
  \begin{mathpar}
  \inferdef{EqType}
    {\Gamma \vdash A \colon \uni[i] \\ \Gamma \vdash a \colon A \\ \Gamma \vdash a' \colon A}
    {\Gamma \vdash \eqty[A]{a}{a'} \colon \uni[i]}
  \label{rule:eq-type}
  \and
  \inferdef{EqRefl}
    {\Gamma \vdash A \colon \uni[i] \\ \Gamma \vdash a \colon A}
    {\Gamma \vdash \refl[A][a] \colon \eqty[A]{a}{a}}
  \label{rule:eq-refl}
  \and
  \inferdef{EqInd}
    {\Gamma \vdash s \colon \eqty[A]{a}{a'} \\
    \Gamma, y \colon A, z \colon \eqty[A]{a}{y} \vdash P \colon \uni \\
    \Gamma \vdash b \colon \multisubs{P}{y \into a, z \into \refl[A][a]}}
    {\Gamma \vdash \ind{\eqtyop}{s}{y.z.P}{b} \colon \multisubs{P}{y \into a', z \into s}}
...
\begin{figure*}
  \begin{mathpar}
    \inferdef{VectType}
      {\vdash A \colon \uni[i] \\ \vdash n \colon \Nat}
      {\Gamma \vdash \Vect(A,n) \colon \uni[i]}
    \label{rule:vect-type} \and
    \inferdef{Vnil}
      {\Gamma \vdash A \colon \uni}
      {\Gamma \vdash \vnil[A] \colon \Vect(A,\z)}
    \label{rule:vnil-type} \and
    \inferdef{Vcons}
      {\Gamma \vdash A \colon \uni \\ \Gamma \vdash n \colon \Nat \\
      \Gamma \vdash a \colon A \\ \Gamma \vdash l \colon \Vect(A,n)}
      {\Gamma \vdash \vcons[A][n]{a}{l} \colon \Vect(A,\S n)}
    \label{rule:vcons-type} \and
    \inferdef{VectInd}
      {\Gamma \vdash s \colon \Vect(A,n) \\
      \Gamma, y \colon \Nat, z \colon \Vect(A,y) \vdash P \colon \uni \\
      \Gamma \vdash b_{\vnil} \colon \multisubs{P}{y \into 0, z \into \vnil} \\
      \Gamma, y_1 \colon \Nat, y_2 \colon A, y_3 \colon \Vect(A,y_1), p_{y_3} \colon \multisubs{P}{y \into y_1, z \into y_3} \vdash
        b_{\vconsop} \colon \multisubs{P}{y \into \S{y_1}, z \into \vcons[A][y_1]{y_2}{y_3}}}
      {\Gamma \vdash \ind{\Vect}{s}{y.z.P}{b_{\vnil},y_1.y_2.y_3.p_{y_3}.b_{\vconsop}}
        \colon \multisubs{P}{y \into n, z \into s}}
...
$\l (A \colon \uni[i]) (x \colon A).\ x$, of type $\P A \colon \uni[i] .\ A \to A$.
...
by allowing $\P x \colon A.\ B \cum \P x \colon A'.\ B$ whenever $A \conv A'$ and $B \cum B'$.
...
\begin{marginfigure}
  \begin{mathpar}
    ...
    \inferdef{ΠCum}{A \conv A' \\ B \cum B'}{\P x \colon A.\ B \cum \P x \colon A'.\ B'}
    ...
    \inferdef{UCum}{\Gamma \vdash T' \colon \uni \\ T \cum T'}{\Gamma \vdash T \cum T' \colon \uni}
      \label{rule:cic-ucum} \and
    \inferdef{Cum}{\Gamma \vdash t \colon T \\ \Gamma \vdash T \cum T' \colon \uni}
    {\Gamma \vdash t \colon T'}
      \label{rule:cic-cum}
  \end{mathpar}
...
\begin{marginfigure}
  \begin{mathpar}
    \inferdef{Prop}{\vdash \Gamma}{\Gamma \vdash \Prop \colon \uni[0]}
    \label{rule:cic-prop} \and
    \inferdef{ΠTyProp}{\Gamma \vdash A \colon \uni \\ \Gamma, x \colon A \vdash P \colon \Prop}
    {\Gamma \vdash \P x \colon A.\ P \colon \Prop}
    \label{rule:prop-prod} \and
    \inferdef{ΠPropProp}{\Gamma \vdash A \colon \Prop \\ \Gamma, x \colon A \vdash P \colon \Prop}
    {\Gamma \vdash \P x \colon A.\ P \colon \Prop}
...
way on the first. For instance, if $P \colon \Prop$, it ensures that it is impossible
to build a function $f \colon P \to \Bool$ and two terms $p_1 \colon P$ and $p_2 \colon P$ such that
...
$\operatorname{head} \colon \P (A \colon \uni) (x \colon \Nat).\ \Vect(A,\S x) \to A$, and consider
\[\letin{x}{\Nat}{1}{\l v \colon \Vect(\Bool,x).\ \operatorname{head}\ \Bool\ 0\ v} \]
...
\begin{figure}
  \ContinuedFloat*
  \begin{mathpar}
    \inferdef{LetIn}{\Gamma \vdash A \colon \uni \\ \Gamma \vdash t \colon A \\
    \Gamma, x := t \colon A \vdash u \colon B}
    {\Gamma \vdash \letin{x}{A}{t}{u} \colon \letin{x}{A}{t}{B}}
...
\begin{marginfigure}
  ...
  \begin{mathpar}
    ...
    \inferdef{δRed}{(x := t \colon A) \in \Gamma}{\Gamma \vdash x \tred t}
...
$\Sigma ;;; \Gamma \vdash t \colon T$, with $\Sigma$ corresponding to the \kl{environment}.
...
$\operatorname{oddS} \colon \P x \colon \Nat.\ \operatorname{even} x \to \operatorname{odd} \S x$ and $\operatorname{evenS} \colon \P x \colon \Nat.\ \operatorname{odd} x \to \operatorname{even} \S x$.
