...
\sidenote[][14em]{
  ...
  another feature that I feel sets apart the tradition around \kl{CIC} from that around
  Martin-Löf type theory: the definition of \kl{conversion} as an \emph{untyped} relation.}
...
with the same name – or \intro{α-equality} $\intro*\alpheq$ – the identification between two terms
...
\intro{one-step reduction} $\intro*\ored$, which allows triggering top-level reduction exactly once,
...
previously defined one $\fred$, we call the latter \intro{full reduction}.
\begin{figure}[ht]
...
\kl{cumulativity}, which is not symmetric – it is a pre-order –, so the more involved
...
\sidenote[][-2em]{At least if one extends its universe hierarchy with an impredicative
universe.}
...
the general class of \intro{inductive types} have been introduced
