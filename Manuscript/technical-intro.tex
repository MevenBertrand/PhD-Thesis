...
but there are two approaches to that. The
first is to simply \emph{define} conversion as an equivalence relation, by adding rules 
...
\end{marginfigure}
This ensures that it indeed has the right properties, but
...
The theoricians of $\l$-calculus have known this issue for a long time, and they
have a solution: characterizing conversion by means of a \kl{reduction} relation $\red$, which
corresponds to the idea of program evaluation. If this reduction has good
...
operational characterization is closer to what can be implemented, which is a good point.
Turning this around, one can define conversion through reduction, and only afterward show
...
\kl{declarative} one we will use to define \kl{CCω}, at it is the most standard;
the \kl{algorithmic} one will appear in \arefpart{metacoq}
to show decidability of type-checking, and in
\arefpart{gradual} because there we want to replace conversion by a non-transitive relation,
and declarative conversion is thus not a good candidate
as it – by design – enforces transitivity.