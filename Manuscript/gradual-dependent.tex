...
\end{coqcode}
is ill-typed, because \coqe{vec A (raise ℕ)} is still not convertible to
\coqe{vec A (S (raise ℕ))}.
The reason is that \coqe{raise ℕ} behaves like an extra constructor to \coqe{ℕ}, so
\coqe{S (raise ℕ)} is itself a normal form,
...
but is \intro{equi-precise} to it – noted $t \equiprecise \asc{\asc{t}{B}}{A}$.
...
\sidenote{
  There exist non-gradual dependently-typed programming languages that admit divergence, \eg \kl{Dependent Haskell} \sidecite[3em]{Eisenberg2016}, \kl{Idris} \sidecite[6em]{Brady2013}. We will also present one such theory in this article. 
}
...
is but a particular \kl{Pure Type System} \sidecite{barendregt:jfp1991}.
Thus, we consider a parametrized version of a gradual \kl{CIC}, called
\kl{GCIC}, with two parameters%
\sidenote{This system is precisely detailed in \cref{fig:ccic-typing}}.
...
The latter option yields a variant of \kl{CIC} that we call \kl{CICs} – read “\kl{CIC}-shift”.
...
\intro{GCICP} is a theory that satisfies both \kl{conservativity} with respect to \kl{CIC}
...
\kl{consistency}. This results in a theory that is not normalizing.

\intro{GCICs} is a theory that satisfies both \kl{normalization} and \kl{graduality},
...
the potential non-termination induced by the use of \kl{consistency}
...
sense, \kl{GCICP} can embed the untyped lambda calculus just as
...
\kl{GCICP} favors flexibility over decidability of type-checking. While this might appear
...
In practice, though, the fact that it is not conservative wrt full \kl{CIC} means that one would
not be able to simply import existing libraries as soon as they
fall outside of the \kl{CICs} subset.
...
\kl{GCICT} is normalizing and able to import existing libraries without restrictions,
...
to a target \intro{cast calculus}, \ie a language with explicit runtime type
checks.
...
named \kl{CastCIC} (\cref{sec:cast-calculus}).
...
are required to satisfy \kl{consistency}. For instance, $\asc{\asc{\true}{\?}}{\Nat}$ is well-typed by \kl{consistency} – used twice –, but $\asc{\true}{\Nat}$ is ill-typed.
...
$\cast{\cast{0}{\Nat}{\?[\uni]}}{\?[\uni]}{\Bool}$ in \kl{CastCIC}.
...
First, it is required in order to tame the non-transitive \kl{consistency} relation.
Indeed, in the previous example of $\asc{\true}{\Nat}$, if one kept a free-standing rule like
\namecref{rule:cic-conv} and simply replaced \kl{conversion} by \kl{consistency}, one could
use the rule twice, through $\?$, and the term would be well-typed.
But \kl{consistency} demands that only terms with explicitly-ascribed
...
\reintro{elaboration graduality} (\cref{thm:static-graduality}), which is
...
%
Because \kl{DGG} is about the behavior of terms during reduction,
