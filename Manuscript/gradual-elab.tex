...
The typing judgment of \kl{GCIC} is \emph{defined}
by an elaboration judgment from \kl{GCIC} to \kl{CCIC}, based upon
that of \arefpart{bidir}, but
augmenting all judgments with an extra output: the elaborated \kl{CCIC} term.
...
\sidenote{We use \sourcecolor{teal} for terms of
  \kl{GCIC}. To maintain a distinction in the absence of colors, we also
...
\begin{definition}[\intro{Consistent conversion}]
	\label{def:cons}
	Two \kl{CCIC} terms are \kl{α-consistent}, written $\acons$,
...
conversions, typically between static terms.
...
The \kl{consistency} relation is then built upon \kl{α-consistency} in a way totally similar to
...
\kl{declarative conversion}, because we do \emph{not} want \kl{consistency} to be transitive,
...
is that it corresponds to \kl{conversion} on static terms.

\begin{proposition}[Properties of \kl{consistent conversion}]
	\label{prop:cons-static}
  \begin{itemize}
  \item Two static terms are consistently convertible if and only if
...
\begin{proof}
  ...
  between static terms corresponds to \kl{α-equality} of terms.
  Thus, and because the \kl{reduction} of static terms in \kl{CCIC}
  is the same as the \kl{reduction} of \kl{CIC},
  two consistent static terms must reduce to \kl{α-equal} terms,
  ...
  for deciding conversion, and relies on iterated \kl{weak-head} normalization,
  can be adapted to decide \kl{consistency}. This would
  however need somewhat subtle proofs, in the vein of those we use to prove the \kl{DGG}.
\end{proof}
...
%
  ...
  \label{rule:gcic-ind}
  \end{mathpar}
  \caption{Type-directed elaboration of \kl{GCIC}: static fragment}
...
Note that only the \kl{subject} of the judgment is a \scol{source term} in \kl{GCIC};
...
\begin{proof}
  ...
  mutually with similar properties for all elaboration judgments.
  It resembles a lot the \kl{correctness} proof for bidirectional typing
...
\begin{proof}
  ...
  Coupled with decidability of \kl{consistency} (\cref{prop:cons-static}),
...
\begin{proof}
  ...
  Thus, only one of \ruleref{rule:gcic-inf-prod} and \ruleref{infrule:gcic-prod-unk} can apply.
...
and explain in detail the behavior described in \cref{sec:gcic:-3-1} for the three
\kl{GCIC} variants. 
...
Let us now look at the reduction behavior of this elaborated term $\tcol{\Omega'}$ in
...
the static gradual guarantee \kl{SGG} in our setting – is no small feat, as it
...
necessary to know how \kl{consistency} and \kl{reduction} evolve as a type
becomes less precise.

As already explained in \cref{sec:gcic-overview}, we
cannot directly prove graduality for a syntactic notion of
precision. However, we can still show that this relation is a \kl{simulation} for reduction.
While weaker than graduality, this property implies the \kl{DGG},
and suffices to conclude that graduality of elaboration holds.
...
and \cref{fig:apre} draws from their Fig. 9, especially for \ruleref{rule:capre-castr}
and \ruleref{infrule:capre-castl}.
...
\kl{syntactic precision}, on terms in \kl{GCIC}, which corresponds to the
usual syntactic precision of gradual typing, such as that of \textcite{Siek2015};
and \kl{structural precision} on terms in \kl{CCIC}, which corresponds
...
\subsection{Structural precision for \kl(tit){CCIC}}
...
\begin{definition}[\kl(pre){Structural} and \kl{definitional} precision in \kl{CCIC}]
...
term on the left – \ruleref{infrule:capre-castr}. Dually, casts on the
...
\end{marginfigure}
%
The situation is the same as for \kl{consistency} – resp.\ \kl{algorithmic conversion} –, which
is the closure by \kl{reduction} of \kl{α-consistency} – resp. \kl{α-equality}.
%
...
The fact that \kl{structural precision} is a simulation relies on a series of lemmas,
...
\begin{proof}
  It is enough to show the property for $\capre$, in which case we prove the judgment
  by induction on $T'$.
...
\begin{proof}
  ...
  \kl{weak-head} normal form of the right kind – either $\?[\uni]$ or $\uni$.
...
and \cref{lem:catchup-lambda} ensures that for those precision is still a simulation,
even in \kl{CCICT}.
...
We finally come to the main property of this section, the advertised
simulation property.
...
\begin{theorem}[Precision is a simulation for reduction]
...
\begin{proof}
  ...
  \kl{top-level}, where we use \cref{lem:catchup-lambda,lem:catchup-cons},
  ...
  Finally, since the catch-up can be done using only \kl{weak-head} reduction,
  a \kl{weak-head} reduction step can always be simulated by \kl{weak-head} reductions.
...
static terms are not elaborated to themselves. Therefore we use a (partial) erasure function $\eras$ to relate terms of \kl{CCIC} to terms of \kl{CIC} by erasing all casts.
We also introduce the notion of \kl{erasability},
characterizing terms that contain only ”harmless“ casts,
such that in particular the elaboration of a static term is always erasable.
...
\begin{definition}[Erasure, erasability]
  \label{def:erasure}
	\kl{Erasure} $\eras$ is a partial function from the syntax of \kl{CCIC} to the syntax of
  ...
  is \intro{erasable} if $\eras(\tcol{t})$ is defined, well-typed in $\tcol{\sn{\GG}}$,
...
\begin{theorem}[\intro{Conservativity}]
	...
	Let $t$ be a static term – \ie a term of \kl{CIC}.
...
\begin{proof}
	...
	Because $\scol{t}$ is static, its typing derivation in \kl{GCIC}
  ...
  \kl{CCIC} are erasable, and more precisely that any static term $t$
  ...
  erasable term $\tcol{t}$ has the same reduction behavior as
  ...
  \kl{constrained reductions} on an erasable term of \kl{CCIC} behave
  the same as its erasure. Similarly \kl{consistency} of erasable terms of \kl{CCIC}
...
\intro(gcic){syntactic precision} $\apre$, defined in \cref{fig:apre-gcic}.
%
\kl(gcic){Syntactic precision} is the usual and expected source-level notion
...
requires an additional hypothesis relating elaboration and precision judgments.

\begin{definition}[Universe adequacy]
  We say that two judgments $\scol{\tilde{t}} \apre \scol{\?[i]}$ and
  $\inferelab{\Gamma}{\tilde{t}}{t}{T}$ are \intro{universe adequate} if the
  universe level $j$ given by the well-formedness judgment
  ...
  subterm $\scol{\tilde{t}_0}$ of $\scol{\tilde{t}}$ inducing judgments $\scol{\tilde{t}_0}
...
\begin{theorem}[Elaboration Graduality / Static Gradual Guarantee]
...
\begin{proof}
  ...
  %
  By \kl{safety} (\cref{thm:ccic-safety}), $\tcol{\mathcal{C}[t]}$ either reduces
...
because in that context the function $\tcol{\l x : \Nat . \S x}$ reduces to $\tcol{\true}$ while the less precise casted function reduces to $\tcol{\err[\Bool]}$.