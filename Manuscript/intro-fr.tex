...
\sidenote{
  Le plus connu est probablement le syllogisme Barbara, dont un exemple est :
...
\sidenote{
  ...
  publics~\sidecite[1.5em][Nachwort p.~253]{Frege1903}.}
...
par Russell et Whitehead dans leur \citetitle{Whitehead1913}~\sidecite[1em]{Whitehead1913},
...
\sidenote{Avec des systèmes comme Automath~\sidecite[1.5em]{DeBruijn1970}
ou Mizar~\sidecite[5.5em]{Rudnicki1992}.}
...
\sidenote{Dans la plupart des assistants à la preuve, la preuve finale est
...
et la question de leur intégration dans les assistants à la
preuve y est un sujet de actif \sidecite{Boehme2010,Ekici2017}.
...
avec les assistants à la preuve commencent à voir le jour \sidecite{Lewis2022,Melquiond2012}.
...
les \intro{types} sont une sorte d’équivalent des catégories grammaticales.
...
à la programmeuse.\sidenote{
  Un slogan dû à \sidetextcite[4em]{Milner1978} est que
...
Une paire $(a,b)$ construite à partir d’un premier objet $a$ de type $A$ et d’un second objet
$b$ de type $B$ a le type $A \times B$.
...
à une correspondance générale entre d’une part les énoncés de la logique et les types des langages de programmation, et d’autre part les preuves d’un énoncé et les programmes ayant le type correspondant.\sidenote{
...
ils ne risquent pas de disparaître de si tôt.
...
\sidenote{C’est celle sur laquelle j’ai principalemente travaillé, et sur laquelle 
on reviendra plus longuement.}
consiste à implémenter un algorithme de vérification de type ressemblant au maximum à celui du \kl{noyau}, directement en \kl{Gallina}\sidenote{
  \color{black}
  En effet, grâce à la \kl{correspondance de Curry-Howard}, \kl{Gallina} est certes un langage
...
Enfin, lors d’une troisième étape, on extrait de ce programme \kl{Gallina} certifié
...
en la programmant à nouveau en \kl{Gallina}.
...
en pouvant choisir finement quelles parties d’un programme on veut vérifier, et à quel moment.
...
Enfin le \cref{chap:beyond-gcic} décrit succintement le reste des travaux auxquels
...
pour le Calcul des Constructions Inductives. Ma contribution technique s’est
...
\cref{chap:beyond-gcic}, ainsi que de la partie technique de \textcite{LennonBertrand2022}
dont le contributeur principal est Kenji.
