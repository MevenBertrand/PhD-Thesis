...
De plus, le développement et la vérification humaine de mathématiques
formalisées%
...
preuve est un domaine de recherche active~\sidecite{Boehme2010,Ekici2017}.
...
est basée sur la \kl{correspondance de Curry-Howard}
...
\sidenote{Pour tout entier $n$, la propriété $P(n)$ est
vérifiée.} et existentielles.%
\sidenote{Il existe un entier $n$ tel que $P(n)$ tient.},
...
est qu’il s’agit un \kl[assistant à la preuve]{\emph{assistant} à la preuve}.
...
qui puisse être utilisé en lieu et place du \kl{noyau} actuel.
